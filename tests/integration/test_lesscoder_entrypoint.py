...
def test_lesscoder_server_invokes_adapter(monkeypatch):
    ...
    assert code == 0
    assert calls["cmd"][:4] == ["cargo", "run", "--manifest-path", "engine/rust/alsp_adapter/Cargo.toml"]
    assert calls["cmd"][-2:] == ["--bin", "alsp_adapter"]
    assert calls["check"] is False
    assert calls["env"]["ALSP_ADAPTER_ADDR"] == "127.0.0.1:8799"
